...
COMMAND="$1" # setup | setup-fork | update
ARG1="$2"
...
if [ "${COMMAND}" == "setup" ]; then
  ...
  git remote add origin "git@github.com:content-services/content-sources-backend.git"
  git switch main
...
if [ "${COMMAND}" == "setup-dev" ]; then
  ...
  git remote add upstream "git@github.com:content-services/content-sources-backend.git"
  git switch main
fi
